...
/**
 * Finds high-confidence alternative command based on user typing or submitted input
 */
export function findAlternativeCommand(input: string, threshold = 0.65): CommandSuggestion | null {
  ...
  for (const item of CANONICAL_JARVIS_COMMANDS) {
    ...
    // If user already typed the exact canonical command, no suggestion needed
    if (cleanInput === canonicalLower) continue;

    for (const keyword of item.keywords) {
      const kw = keyword.toLowerCase();
      // Direct exact keyword match that differs from full canonical command
      ...
      // Keyword substring / prefix match
